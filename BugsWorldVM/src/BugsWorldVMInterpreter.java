...
public final class BugsWorldVMInterpreter {
	 ...
	 * HALT).
	 *
	 * @param byteCode the integer to be checked
	 * @return true if {@code byteCode} is the byte code of a primitive instruction
	 *         or false otherwise
	 * @ensures
	 * 
	 *          <pre>
	...
	private static boolean isPrimitiveInstructionByteCode(int byteCode) {
		return (byteCode == Instruction.MOVE.byteCode()) || (byteCode == Instruction.TURNLEFT.byteCode())
				|| (byteCode == Instruction.TURNRIGHT.byteCode()) || (byteCode == Instruction.INFECT.byteCode())
				|| (byteCode == Instruction.SKIP.byteCode()) || (byteCode == Instruction.HALT.byteCode());
	}
	 ...
	 *
	 * @param wbs      the {@code CellState} indicating what the bug sees
	 * @param condJump the byte code of a conditional jump
	...
	private static boolean conditionalJumpCondition(CellState wbs, int condJump) {
		...
		} else if (condJump == Instruction.JUMP_IF_NOT_NEXT_IS_NOT_EMPTY.byteCode()) {
			answer = (wbs != CellState.EMPTY);
		} else if (condJump == Instruction.JUMP_IF_NOT_NEXT_IS_WALL.byteCode()) {
			answer = (wbs == CellState.WALL);
		} else if (condJump == Instruction.JUMP_IF_NOT_NEXT_IS_NOT_WALL.byteCode()) {
			answer = (wbs != CellState.WALL);
		} else if (condJump == Instruction.JUMP_IF_NOT_NEXT_IS_FRIEND.byteCode()) {
			answer = (wbs == CellState.FRIEND);
		} else if (condJump == Instruction.JUMP_IF_NOT_NEXT_IS_NOT_FRIEND.byteCode()) {
			answer = (wbs != CellState.FRIEND);
		} else if (condJump == Instruction.JUMP_IF_NOT_NEXT_IS_ENEMY.byteCode()) {
			answer = (wbs == CellState.ENEMY);
		} else if (condJump == Instruction.JUMP_IF_NOT_NEXT_IS_NOT_ENEMY.byteCode()) {
			answer = (wbs != CellState.ENEMY);
		...
		return answer; // answer TRUE 时不会跳。
	}
		...
		while ((pos < cp.length) && !found) {
			if (pos == loc) {
				...
				if (!isPrimitiveInstructionByteCode(cp[pos])) {
					/*
					 * It must be a jump instruction, increment pos one extra time
					 */
					pos++;
				}
				pos++;
	...
	/**
	 * Returns the location of the next primitive instruction to execute in compiled
	 * program {@code cp} given what the bug sees {@code wbs} and starting from
	 * location {@code pc}.
	 *
	 ...
	 *          </pre>
	 */
	public static int nextPrimitiveInstructionAddress(int[] cp, CellState wbs, int pc) {
		...
		int loc = pc;
		while (!isPrimitiveInstructionByteCode(cp[loc])) {
			if (cp[loc] == Instruction.JUMP.byteCode() || !conditionalJumpCondition(wbs, cp[loc])) {
				loc = cp[loc + 1];
			} else {
				loc += 2;
			}
	...
	public static void main(String[] args) {
		...
		int pc = 0;
		out.println();
		out.println("Enter program counter outside the [0," + cp.length + ") range to quit.");
		/*
		...
		while (true) {
			...
			if (input.length() > 0 && !FormatChecker.canParseInt(input)) {
				out.println("Program counter must be a number in the [0," + cp.length + ") range");
				continue;
				...
				if (pcCandidate < 0 || pcCandidate >= cp.length) {
					...
					out.println("Program counter must be the location of an " + "instruction byte code in the program");
					continue;
				} else {
					pc = pcCandidate;
				}
			 ...
			 */
			out.print("Enter what bug sees " + "(EMPTY=0, WALL=1, FRIEND=2, ENEMY=3): ");
			...
			default: {
				out.println("What bug sees must be a number" + " in the [0,3] range");
				continue;
			}
			}
			/*
			...
			out.println();
			out.println("  Next primitive instruction: " + Program.Instruction.values()[cp[pc]].toString()
					+ " at address " + pc);
			/*
			 * Output disassembled program with marked address
			 */
